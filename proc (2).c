...
found:
    ...
    p->pid = nextpid++;
    p->tidCounter = 1;
    //TODO-initial the proc lock
    initlock(p->procLock, (char *) p->pid);
...
foundThread:
    ...
    if ((p->mainThread->tkstack = kalloc()) == 0) {
        p->state = UNUSED;
        return 0;
    }
    sp = p->mainThread->tkstack + KSTACKSIZE;
...
fork(void) {
    ...
    np->sz = curproc->sz;
    np->parent = curproc;
    //TODO - maybe need curproc->mainThread
...
exit(void) {
    ...
    for (t = curproc->thread; t < &curproc->thread[NTHREADS]; t++) {
        ...
        if(t != curthread)
            ...
            //sleep(t, curproc->procLock);  //DOC: wait-sleep
    }
...
//    struct  thread *t;
    ...
    acquire(p->procLock);
    //TODO - run over the ttable and change all the thread states
//    for (t = p->thread; t < &p->thread[NTHREADS]; t++){
//        if(t->state != UNUSED)
//        t->state = RUNNABLE;
//    }
